...
const mapReporteRow = (r, idx) => ({
  ...
  'CULTO ORAC.': r.cantHrOracion || 0,
  'MEP': r.cantHrMep || 0,
  'DISCIP.': r.cantHrDiscipulado || 0,
...
export const exportConsolidadoExcel = (reportes, titulo = 'Consolidado', fileName = 'Consolidado') => {
  ...
  const total = {
    ...
    'CULTO ORAC.': sumField(reportes, 'cantHrOracion'),
    'MEP': sumField(reportes, 'cantHrMep'),
    'DISCIP.': sumField(reportes, 'cantHrDiscipulado'),
...
export const exportConsolidadoPDF = (reportes, titulo = 'Consolidado Sectorial', fileName = 'Consolidado') => {
  ...
  const headers = [
    '#', 'LÍDERES',
    'DIEZMO', 'LECT. BIB.', 'ORACIÓN',
    'VISITO', 'AYUNO', 'HERMAN.', 'AMIGOS', 'ADOLES.', 'TOTAL',
    'CONVERT.', 'NIÑOS CRIST.', 'NIÑOS AMIGOS', 'TOTAL',
    'V. CONSOL.', 'V. CASA PAZ', 'V. HOGAR',
    'CULTO ORAC.', 'MEP', 'DISCIP.', 'RETIRO', 'CULTO CENT.',
    'O. SAB.', 'O. NIÑOS', 'O. MIÉRC.', 'TOTAL OFR.',
  ];

  const bodyRows = reportes.map((r, i) => [
    i + 1,
    r.liderNombre || `Líder ${r.liderId || ''}`,
    fmtBool2(r.diezmo), fmtBool2(r.lecturaBiblia), fmtOracion(r.horasOracion, r.minutosOracion),
    fmtBool2(r.visito), fmtBool2(r.ayuno),
    ...
    Number(r.ofrendaSabado || 0).toFixed(2),
    Number(r.ofrendaNinos || 0).toFixed(2),
    Number(r.ofrendaMiercoles || 0).toFixed(2),
    (Number(r.ofrendaSabado || 0) + Number(r.ofrendaNinos || 0) + Number(r.ofrendaMiercoles || 0)).toFixed(2),
  ]);

  const totalRow = [
    '', 'TOTAL',
    sumField(reportes, 'diezmo'), sumField(reportes, 'lecturaBiblia'), sumarOracion(reportes, 'horasOracion', 'minutosOracion'),
    sumField(reportes, 'visito'), sumField(reportes, 'ayuno'),
    ...
    reportes.reduce((a, r) => a + Number(r.ofrendaSabado || 0), 0).toFixed(2),
    reportes.reduce((a, r) => a + Number(r.ofrendaNinos || 0), 0).toFixed(2),
    reportes.reduce((a, r) => a + Number(r.ofrendaMiercoles || 0), 0).toFixed(2),
    reportes.reduce((a, r) => a + Number(r.ofrendaSabado || 0) + Number(r.ofrendaNinos || 0) + Number(r.ofrendaMiercoles || 0), 0).toFixed(2),
  ];
  ...
  autoTable(doc, {
    ...
    columnStyles: {
      ...
      21: { halign: 'center' },
      22: { halign: 'center' },
      23: { halign: 'right', cellWidth: 18 },
      24: { halign: 'right', cellWidth: 18 },
      25: { halign: 'right', cellWidth: 18 },
      26: { halign: 'right', cellWidth: 18 },
    },
  });
...
export const exportConsolidadoSectoresExcel = (sectoresData, titulo = 'Consolidado por Sectores', fileName = 'ConsolidadoSectores') => {
  ...
  const rows = sectoresData.map((s, idx) => {
    ...
    return {
      ...
      'CULTO ORAC.': sumField(reps, 'cantHrOracion'),
      'MEP': sumField(reps, 'cantHrMep'),
      'DISCIP.': sumField(reps, 'cantHrDiscipulado'),
  ...
  const totalRow = {
    ...
    'CULTO ORAC.': rows.reduce((a, r) => a + r['HR ORACIÓN'], 0),
    'MEP': rows.reduce((a, r) => a + r['HR MEP'], 0),
    'DISCIP.': rows.reduce((a, r) => a + r['HR DISCIP.'], 0),
